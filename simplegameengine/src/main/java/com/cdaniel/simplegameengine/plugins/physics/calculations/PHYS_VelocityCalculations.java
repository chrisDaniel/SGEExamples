 ...
 */
public class PHYS_VelocityCalculations {


    public static void addVelocity(Vector velocity, Vector delta){

        float dX = delta.getEx() - delta.getSx();
        float dY = delta.getEy() - delta.getSy();
        float dZ = delta.getEz() - delta.getSz();

        velocity.transform(velocity.getSx(), velocity.getSy(), velocity.getSz(),
                            velocity.getEx()+dX, velocity.getEy()+dY, velocity.getEz()+dZ);
    }
